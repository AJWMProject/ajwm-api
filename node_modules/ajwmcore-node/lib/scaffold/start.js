...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for ajwmd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and ajwmcore-node.json. \n\n' +
...
/**
 * This function will insajwmiate and start a Node, requiring the necessary service
 ...
 * @param {Object} options.config - The parsed ajwmcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './ajwmcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in ajwmcore-node services directory
    ...
    if (servicePackage.ajwmcoreNode) {
      serviceModule = service.name + '/' + servicePackage.ajwmcoreNode;
    }
