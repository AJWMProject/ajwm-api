...
var bitcore = require('ajwmcore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var AjwmRPC = require('ajwmd-rpc');
...
/**
 * Provides a friendly event driven API to ajwmd in Node.js. Manages starting and
 * stopping ajwmd as a child process for application support, as well as connecting
 * to multiple ajwmd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Ajwm(options) {
  if (!(this instanceof Ajwm)) {
    return new Ajwm(options);
  }
...
util.inherits(Ajwm, Service);

Ajwm.dependencies = [];

Ajwm.DEFAULT_MAX_TXIDS = 1000;
Ajwm.DEFAULT_MAX_HISTORY = 50;
Ajwm.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Ajwm.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Ajwm.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Ajwm.DEFAULT_SPAWN_RESTART_TIME = 5000;
Ajwm.DEFAULT_SPAWN_STOP_TIME = 10000;
Ajwm.DEFAULT_TRY_ALL_INTERVAL = 1000;
Ajwm.DEFAULT_REINDEX_INTERVAL = 10000;
Ajwm.DEFAULT_START_RETRY_INTERVAL = 5000;
Ajwm.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Ajwm.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Ajwm.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Ajwm.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Ajwm.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Ajwm.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Ajwm.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Ajwm.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Ajwm.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Ajwm.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Ajwm.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Ajwm.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Ajwm.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Ajwm.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Ajwm.prototype._initCaches = function() {
...
Ajwm.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Ajwm.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Ajwm.prototype.getPublishEvents = function() {
  return [
    {
...
Ajwm.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'ajwmd/' + name, 'total:', this.subscriptions[name].length);
};

Ajwm.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'ajwmd/' + name, 'total:', this.subscriptions[name].length);
};


Ajwm.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'ajwmcoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Ajwm.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'ajwmcoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Ajwm.prototype.unsubscribeBalanceAll = function(emitter) {
...
Ajwm.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'ajwmd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Ajwm.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'ajwmd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Ajwm.prototype.unsubscribeAddressAll = function(emitter) {
...
Ajwm.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Ajwm.DEFAULT_CONFIG_SETTINGS;
...
Ajwm.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Ajwm.prototype._expandRelativeDatadir = function() {
...
Ajwm.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in ajwmd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in ajwmd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in ajwmd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './ajwm.conf');

  log.info('Using ajwm.config file:', configPath);
...
Ajwm.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of ajwmcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of ajwmcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of ajwmcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to ajwmd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from ajwmd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from ajwmd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that ajwmd is undergoing a reindex. ' +
             ...
             'of ajwmcore-node services will start.');
    node._reindex = true;
  }
};

Ajwm.prototype._resetCaches = function() {
...
Ajwm.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Ajwm.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Ajwm.prototype._initChain = function(callback) {
  var self = this;
...
Ajwm.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9921
  };
  if (this.node.network === bitcore.Networks.testnet) {
    networkOptions.rpcport = 19921;
  }
  return networkOptions;
};

Ajwm.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/ajwm.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/ajwm.conf';
    }
  }
  return networkPath;
};

Ajwm.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Ajwm.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('ajwmd/hashblock', message.toString('hex'));
    }
  }

};

Ajwm.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Ajwm.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Ajwm Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Ajwm.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Ajwm.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Ajwm.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('ajwmcoind/addressbalance', {
                    address: address,
                    txid: txid,
...
Ajwm.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('ajwmd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Ajwm.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('ajwmd/rawtransaction', message.toString('hex'));
    }
...
Ajwm.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Ajwm.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Ajwm.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Ajwm.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Ajwm.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Ajwm Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Ajwm.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Ajwm.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Ajwm.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/ajwmd.pid';
...
Ajwm.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting ajwm process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Ajwm process unexpectedly exited with code:', code);
        log.warn('Restarting ajwm child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Ajwm process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn ajwmd.'));
      }
...
Ajwm.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to ajwmd.'));
    }
 ...
 */
Ajwm.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Ajwm configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Ajwm.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Ajwm.prototype.syncPercentage = function(callback) {
  var self = this;
...
Ajwm.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, ajwmcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Ajwm.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Ajwm.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Ajwm.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Ajwm.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in ajwmcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Ajwm.prototype._getConfirmationsDetail = function(transaction) {
...
Ajwm.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Ajwm.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Ajwm.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A ajwmcoin transaction id
 * @param {Function} callback
 */
Ajwm.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Ajwm.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Ajwm.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Ajwm.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Ajwm.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Ajwmcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Ajwm.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getInfo = function(callback) {
...
Ajwm.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('ajwmd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('ajwmd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Ajwm.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Ajwm.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Ajwm.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Ajwm.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Ajwm.prototype.getStakingInfo = function(callback) {
    var self = this,
