...
var Transaction = ajwmcore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/ajwm.conf')));
var AjwmService = proxyquire('../../lib/services/ajwmd', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultAjwmConf = fs.readFileSync(path.resolve(__dirname, '../data/default.ajwm.conf'), 'utf8');

describe('Ajwm Service', function() {
	...
	var baseConfig = {
		node: {
			network: ajwmcore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var ajwmd = new AjwmService(baseConfig);
			should.exist(ajwmd);
		});
		it('will create an instance without `new`', function() {
			var ajwmd = AjwmService(baseConfig);
			should.exist(ajwmd);
		});
		it('will init caches', function() {
			var ajwmd = new AjwmService(baseConfig);

			should.exist(ajwmd.utxosCache);
			should.exist(ajwmd.txidsCache);
			should.exist(ajwmd.balanceCache);
			should.exist(ajwmd.summaryCache);
			should.exist(ajwmd.transactionDetailedCache);
			should.exist(ajwmd.accountInfo);

			should.exist(ajwmd.transactionCache);
			should.exist(ajwmd.rawTransactionCache);
			should.exist(ajwmd.rawJsonTransactionCache);
			should.exist(ajwmd.blockCache);
			should.exist(ajwmd.blockJsonCache);
			should.exist(ajwmd.blockSubsidyCache);
			should.exist(ajwmd.rawBlockCache);
			should.exist(ajwmd.blockHeaderCache);
			should.exist(ajwmd.zmqKnownTransactions);
			should.exist(ajwmd.zmqKnownBlocks);
			should.exist(ajwmd.lastTip);
			should.exist(ajwmd.lastTipTimeout);
			should.equal(ajwmd.dgpInfoCache, null, 'should be null');
			should.equal(ajwmd.miningInfoCache, null, 'should be null');
			should.equal(ajwmd.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(ajwmd.maxTxids, 1000);
			should.equal(ajwmd.maxTransactionHistory, 50);
			should.equal(ajwmd.maxAddressesQuery, 10000);
			should.equal(ajwmd.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(ajwmd.spawnRestartTime, 5000);
			should.equal(ajwmd.spawnStopTime, 10000);

			// try all interval
			should.equal(ajwmd.tryAllInterval, 1000);
			should.equal(ajwmd.startRetryInterval, 5000);

			// rpc limits
			should.equal(ajwmd.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(ajwmd.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.nodes.should.deep.equal([]);
			ajwmd.nodesIndex.should.equal(0);
			ajwmd.nodes.push({ client: sinon.stub() });
			should.exist(ajwmd.client);
		});
		it('will set subscriptions', function() {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd._initDefaults({ transactionConcurrency: 10 });
			ajwmd.transactionConcurrency.should.equal(10);
			ajwmd._initDefaults({});
			ajwmd.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			AjwmService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var ajwmd = new AjwmService(baseConfig);
			var methods = ajwmd.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var ajwmd = new AjwmService(baseConfig);
			var events = ajwmd.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('ajwmd/rawtransaction');
			events[0].scope.should.equal(ajwmd);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('ajwmd/hashblock');
			events[1].scope.should.equal(ajwmd);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('ajwmd/addresstxid');
			events[2].scope.should.equal(ajwmd);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('ajwmd/addressbalance');
			events[3].scope.should.equal(ajwmd);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.subscribe = sinon.stub();
			ajwmd.unsubscribe = sinon.stub();
			var events = ajwmd.getPublishEvents();

			events[0].subscribe('test');
			ajwmd.subscribe.args[0][0].should.equal('rawtransaction');
			ajwmd.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			ajwmd.unsubscribe.args[0][0].should.equal('rawtransaction');
			ajwmd.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			ajwmd.subscribe.args[1][0].should.equal('hashblock');
			ajwmd.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			ajwmd.unsubscribe.args[1][0].should.equal('hashblock');
			ajwmd.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var ajwmd = new AjwmService(baseConfig);
			var emitter = {};
			ajwmd.subscribe('hashblock', emitter);
			ajwmd.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			ajwmd.subscribe('rawtransaction', emitter2);
			ajwmd.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			ajwmd.subscribe('hashblock', emitter1);
			ajwmd.subscribe('hashblock', emitter2);
			ajwmd.subscribe('hashblock', emitter3);
			ajwmd.subscribe('hashblock', emitter4);
			ajwmd.subscribe('hashblock', emitter5);
			ajwmd.subscriptions.hashblock.length.should.equal(5);

			ajwmd.unsubscribe('hashblock', emitter3);
			ajwmd.subscriptions.hashblock.length.should.equal(4);
			ajwmd.subscriptions.hashblock[0].should.equal(emitter1);
			ajwmd.subscriptions.hashblock[1].should.equal(emitter2);
			ajwmd.subscriptions.hashblock[2].should.equal(emitter4);
			ajwmd.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var ajwmd = new AjwmService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			ajwmd.subscriptions.hashblock = [emitter1];
			ajwmd.unsubscribe('hashblock', emitter3);
			ajwmd.subscriptions.hashblock.length.should.equal(1);
			ajwmd.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			ajwmd.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(ajwmd.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var ajwmd = new AjwmService(baseConfig);
			var emitter = new EventEmitter();
			ajwmd.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var ajwmd = new AjwmService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ajwmd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			ajwmd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			ajwmd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ajwmd.unsubscribeAddress(emitter1);
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ajwmd.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			ajwmd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ajwmd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ajwmd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(ajwmd, 'unsubscribeAddressAll');
			ajwmd.unsubscribeAddress(emitter1);
			ajwmd.unsubscribeAddressAll.callCount.should.equal(1);
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ajwmd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ajwmd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			ajwmd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			ajwmd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			ajwmd.unsubscribeAddressAll(emitter1);
			ajwmd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ajwmd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			ajwmd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(ajwmd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var ajwmd = new AjwmService(baseConfig);
			var config = ajwmd._getDefaultConfig();
			config.should.equal(defaultAjwmConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a ajwm.conf file', function() {
			var TestAjwm = proxyquire('../../lib/services/ajwmd', {
				fs: {
					readFileSync: readFileSync,
			...
			var ajwmd = new TestAjwm(baseConfig);
			ajwmd.options.spawn.datadir = '/tmp/.ajwm';

			sinon.spy(ajwmd, '_expandRelativeDatadir');
			sinon.spy(ajwmd, '_checkConfigIndexes');
			sinon.spy(ajwmd, '_getDefaultConf')
			sinon.spy(ajwmd, '_getNetworkConfigPath');
			sinon.spy(ajwmd, '_parseBitcoinConf');

			var node = {};
			ajwmd._loadSpawnConfiguration(node);

			ajwmd._expandRelativeDatadir.callCount.should.equal(1);
			ajwmd._checkConfigIndexes.callCount.should.equal(1);
			ajwmd._getDefaultConf.callCount.should.equal(1);
			ajwmd._getNetworkConfigPath.callCount.should.equal(1);
			ajwmd._parseBitcoinConf.callCount.should.equal(2);

			should.exist(ajwmd.spawn.config);
			ajwmd.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: ajwmcore.Networks.testnet,
					configPath: '/tmp/.ajwmcore/ajwmcore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			ajwmd.options.spawn.datadir = './data';
			var node = {};
			ajwmd._loadSpawnConfiguration(node);
			...
			ajwmd.options.spawn.datadir.should.equal('/tmp/.ajwmcore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestAjwm = proxyquire('../../lib/services/ajwmd', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badajwm.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var ajwmd = new TestAjwm(baseConfig);
			(function() {
				ajwmd._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(ajwmcore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the ajwm.conf file', function() {
			var ajwmd = new AjwmService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				ajwmd._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the ajwm.conf file', function() {
			var ajwmd = new AjwmService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the ajwm.conf file', function() {
			var ajwmd = new AjwmService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the ajwm.conf file', function() {
			var ajwmd = new AjwmService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the ajwm.conf file', function() {
			var ajwmd = new AjwmService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				ajwmd.transactionDetailedCache.set(keys[i], {});
				ajwmd.utxosCache.set(keys[i], {});
				ajwmd.accountInfo.set(keys[i], {});
				ajwmd.txidsCache.set(keys[i], {});
				ajwmd.balanceCache.set(keys[i], {});
				ajwmd.summaryCache.set(keys[i], {});
				ajwmd.blockOverviewCache.set(keys[i], {});
				ajwmd.blockJsonCache.set(keys[i], {});
			}
			ajwmd._resetCaches();
			should.equal(ajwmd.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(ajwmd.utxosCache.get(keys[0]), undefined);
			should.equal(ajwmd.accountInfo.get(keys[0]), undefined);
			should.equal(ajwmd.txidsCache.get(keys[0]), undefined);
			should.equal(ajwmd.balanceCache.get(keys[0]), undefined);
			should.equal(ajwmd.summaryCache.get(keys[0]), undefined);
			should.equal(ajwmd.blockOverviewCache.get(keys[0]), undefined);
			should.equal(ajwmd.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			ajwmd._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				ajwmd.nodes[0].client.getInfo.callCount.should.equal(1);
				ajwmd.nodes[1].client.getInfo.callCount.should.equal(1);
				ajwmd.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			ajwmd._tryAllClients(function(client, next) {
				...
				ajwmd.nodes[1].client.getInfo.callCount.should.equal(1);
				ajwmd.nodes[2].client.getInfo.callCount.should.equal(1);
				ajwmd.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert ajwmd-rpc error object into JavaScript error', function() {
			var ajwmd = new AjwmService(baseConfig);
			var error = ajwmd._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			ajwmd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			ajwmd.nodes.push({
				client: {
			...
			ajwmd._initChain(function() {
				log.info.callCount.should.equal(1);
				ajwmd.getRawBlock.callCount.should.equal(1);
				ajwmd.getRawBlock.args[0][0].should.equal('genesishash');
				ajwmd.height.should.equal(5000);
				ajwmd.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			ajwmd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			ajwmd._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			ajwmcore.Networks.disableRegtest();
			baseConfig.node.network = ajwmcore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: ajwmcore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var ajwmd = new AjwmService(config);
			ajwmd._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var ajwmd = new AjwmService(config);
			ajwmd._getDefaultConf().rpcport.should.equal(18332);
		});
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var ajwmd = new AjwmService(config);
			should.equal(ajwmd._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var ajwmd = new AjwmService(config);
			ajwmd._getNetworkConfigPath().should.equal('testnet3/ajwm.conf');
		});
		it('will get default rpc port for regtest', function() {
			ajwmcore.Networks.enableRegtest();
			var config = {
			...
			var ajwmd = new AjwmService(config);
			ajwmd._getNetworkConfigPath().should.equal('regtest/ajwm.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.node.network = ajwmcore.Networks.testnet;
			ajwmd._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.node.network = ajwmcore.Networks.testnet;
			ajwmcore.Networks.enableRegtest();
			ajwmd._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.node.network = ajwmcore.Networks.livenet;
			ajwmcore.Networks.enableRegtest();
			should.equal(ajwmd._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			ajwmd._rapidProtectedUpdateTip = sinon.stub();
			ajwmd.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			ajwmd._zmqBlockHandler(node, message);
			ajwmd._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			ajwmd._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(ajwmd, 'emit');
			ajwmd.on('block', function(block) {
				block.should.equal(message);
				ajwmd.emit.callCount.should.equal(1);
				ajwmd._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			ajwmd._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			ajwmd._zmqBlockHandler(node, message);
			ajwmd._rapidProtectedUpdateTip.callCount.should.equal(1);
			ajwmd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			ajwmd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(ajwmd, 'emit');
			var emitter = new EventEmitter();
			ajwmd.subscriptions.hashblock.push(emitter);
			emitter.on('ajwmd/hashblock', function(blockHash) {
				ajwmd._rapidProtectedUpdateTip.callCount.should.equal(1);
				ajwmd.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				ajwmd._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.syncPercentage = sinon.stub();
			sinon.spy(ajwmd, '_resetCaches');
			ajwmd.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				ajwmd._resetCaches.callCount.should.equal(1);
			...
			ajwmd._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			ajwmd.on('synced', function() {
				ajwmd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			ajwmd.on('synced', function() {
				ajwmd.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			ajwmd.on('error', function(err) {
				log.error.callCount.should.equal(1);
				ajwmd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.syncPercentage = sinon.stub();
			ajwmd._resetCaches = sinon.stub();
			ajwmd.on('tip', function(height) {
				ajwmd._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				ajwmd.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using ajwmcore.Transaction', function() {
			var ajwmd = new AjwmService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = ajwmcore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(ajwmcore.Networks.testnet);
			var outputAddress = ajwmcore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = ajwmcore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: ajwmcore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = ajwmd._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addOutput(ajwmcore.Transaction.Output({
				script: ajwmcore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = ajwmd._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var ajwmd = new AjwmService(baseConfig);
			var tx = ajwmcore.Transaction();
			var address = ajwmcore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(ajwmcore.Transaction.Output({
				script: ajwmcore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(ajwmcore.Transaction.Output({
				script: ajwmcore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			ajwmd._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			ajwmd.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('ajwmd/addresstxid', function(data) {
			...
			ajwmd._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			ajwmd.subscriptions.rawtransaction.push(emitter);
			emitter.on('ajwmd/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			ajwmd._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(ajwmd, 'emit');
			ajwmd.subscriptions.rawtransaction.push(emitter);
			emitter.on('ajwmd/rawtransaction', function() {
				done();
			});
			var node = {};
			ajwmd._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd._updateTip = sinon.stub();
			ajwmd._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			ajwmd.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			ajwmd._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				ajwmd._updateTip.callCount.should.equal(11);
				ajwmd._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			ajwmd._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				ajwmd._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			ajwmd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				ajwmd._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			ajwmd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				ajwmd._zmqBlockHandler.callCount.should.equal(0);
				ajwmd._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var ajwmd = new AjwmService(baseConfig);
			var node = {};
			ajwmd._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			ajwmd._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			ajwmd._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			ajwmd._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestAjwmService = proxyquire('../../lib/services/ajwmd', {
				fs: {
					readFile: readFile
				}
			});
			var ajwmd = new TestAjwmService(baseConfig);
			ajwmd.spawnStopTime = 1;
			ajwmd._process = {};
			ajwmd._process.kill = sinon.stub();
			ajwmd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				ajwmd._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			ajwmd._process.kill = sinon.stub().throws(error2);
			ajwmd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd._loadSpawnConfiguration = sinon.stub();
			ajwmd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			ajwmd._spawnChildProcess(function(err) {
				ajwmd._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd._loadSpawnConfiguration = sinon.stub();
			ajwmd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			ajwmd._spawnChildProcess(function(err) {
				ajwmd._loadSpawnConfiguration.callCount.should.equal(1);
				ajwmd._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var ajwmd = new TestAjwmService(config);
			ajwmd.spawn = {};
			ajwmd._loadSpawnConfiguration = sinon.stub();
			ajwmd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			ajwmd.node.stopping = true;
			ajwmd._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			ajwmd._loadSpawnConfiguration = sinon.stub();
			ajwmd.spawn = {};
			ajwmd.spawn.exec = 'testexec';
			ajwmd.spawn.configPath = 'testdir/ajwm.conf';
			ajwmd.spawn.datadir = 'testdir';
			ajwmd.spawn.config = {};
			ajwmd.spawn.config.rpcport = 20001;
			ajwmd.spawn.config.rpcuser = 'ajwm';
			ajwmd.spawn.config.rpcpassword = 'password';
			ajwmd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			ajwmd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			ajwmd._initZmqSubSocket = sinon.stub();
			ajwmd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			ajwmd._checkReindex = sinon.stub().callsArgWith(1, null);
			ajwmd._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/ajwm.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				ajwmd._loadTipFromNode.callCount.should.equal(1);
				ajwmd._initZmqSubSocket.callCount.should.equal(1);
				should.exist(ajwmd._initZmqSubSocket.args[0][0].client);
				ajwmd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				ajwmd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(ajwmd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn ajwmd spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestAjwmService = proxyquire('../../lib/services/ajwmd', {
				fs: {
			...
			ajwmd._loadSpawnConfiguration = sinon.stub();
			ajwmd.spawn = {};
			ajwmd.spawn.exec = 'ajwmd';
			ajwmd.spawn.datadir = '/tmp/ajwm';
			ajwmd.spawn.configPath = '/tmp/ajwm/ajwm.conf';
			ajwmd.spawn.config = {};
			ajwmd.spawnRestartTime = 1;
			ajwmd._loadTipFromNode = sinon.stub().callsArg(1);
			ajwmd._initZmqSubSocket = sinon.stub();
			ajwmd._checkReindex = sinon.stub().callsArg(1);
			ajwmd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			ajwmd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(ajwmd, '_spawnChildProcess');
			ajwmd._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						ajwmd._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			ajwmd._spawnChildProcess(function(err) {
				...
				ajwmd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				ajwmd.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn ajwmd spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestAjwmService = proxyquire('../../lib/services/ajwmd', {
				fs: {
			...
			ajwmd._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						ajwmd._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			ajwmd.spawn.config.rpcpassword = 'password';
			ajwmd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			ajwmd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			ajwmd._spawnChildProcess(function(err) {
				ajwmd._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			ajwmd._initZmqSubSocket = sinon.stub();
			ajwmd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			ajwmd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var ajwmd = new AjwmService(config);
			ajwmd.node.stopping = true;
			ajwmd.startRetryInterval = 100;
			ajwmd._loadTipFromNode = sinon.stub();
			ajwmd._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to ajwmd/);
				ajwmd._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			ajwmd.startRetryInterval = 1;
			var config = {};
			ajwmd._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				ajwmd._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			ajwmd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			ajwmd._connectProcess(config, function(err, node) {
				should.not.exist(err);
				ajwmd._loadTipFromNode.callCount.should.equal(1);
				ajwmd._initZmqSubSocket.callCount.should.equal(1);
				ajwmd._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.options = {};
			ajwmd.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('Ajwm configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			ajwmd.options = {
				connect: [
					{}
				]
			...
			ajwmd.start(function(err) {
				ajwmd._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			var node = {};
			ajwmd._initChain = sinon.stub().callsArg(0);
			ajwmd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			ajwmd.options = {
				spawn: {}
			};
			ajwmd.start(function(err) {
				should.not.exist(err);
				ajwmd.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(ajwmd, '_spawnChildProcess');
			var nodes = [{}];
			ajwmd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			ajwmd.options = {
				connect: [
					{}
				]
			...
			ajwmd.start(function(err) {
				should.not.exist(err);
				ajwmd._initChain.callCount.should.equal(1);
				ajwmd._connectProcess.callCount.should.equal(1);
				ajwmd.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			ajwmd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				ajwmd.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			ajwmd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			ajwmd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			ajwmd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.syncPercentage(function(err) {
				should.exist(err);
				ajwmd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			ajwmd.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				ajwmd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var ajwmd = new AjwmService(baseConfig);
			var args = ajwmd._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var ajwmd = new AjwmService(baseConfig);
			var args = ajwmd._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			sinon.spy(ajwmd, '_normalizeAddressArg');
			ajwmd.nodes.push({
				client: {
			...
			var options = {};
			ajwmd.getAddressBalance(address, options, function(err) {
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			ajwmd.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				ajwmd.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					ajwmd._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			ajwmd.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = ajwmd._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = ajwmd._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = ajwmd._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = ajwmd._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				ajwmd._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			ajwmd.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(ajwmd, '_getHeightRangeQuery');
			sinon.spy(ajwmd, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			ajwmd.getAddressTxids(address, options, function(err) {
				should.exist(err);
				ajwmd._getHeightRangeQuery.callCount.should.equal(1);
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			ajwmd.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				ajwmd._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			ajwmd.getAddressTxids(address, options, function(err, txids) {
				...
				ajwmd.getAddressTxids(address, options, function(err, txids) {
					...
					ajwmd._getHeightRangeQuery.callCount.should.equal(2);
					ajwmd._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			ajwmd.getAddressTxids(address, options, function(err, txids) {
				...
				ajwmd.getAddressTxids(address, options, function(err, txids) {
					...
					ajwmd._getHeightRangeQuery.callCount.should.equal(4);
					ajwmd._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			ajwmd.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				ajwmd.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					ajwmd.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						ajwmd._getHeightRangeQuery.callCount.should.equal(3);
						ajwmd._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.height = 10;
			var confirmations = ajwmd._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var ajwmd = new AjwmService(baseConfig);
			var result = {};
			ajwmd._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var ajwmd = new AjwmService(baseConfig);
			var result = {};
			ajwmd._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will insajwmiate if outputIndexes not defined', function() {
			var ajwmd = new AjwmService(baseConfig);
			var result = {
				addresses: {},
				satoshis: 10,
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var ajwmd = new AjwmService(baseConfig);
			var result = {};
			ajwmd._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var ajwmd = new AjwmService(baseConfig);
			var result = {};
			ajwmd._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will insajwmiate if outputIndexes not defined with', function() {
			var ajwmd = new AjwmService(baseConfig);
			var result = {
				addresses: {},
				satoshis: 10,
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(ajwmd, '_getAddressDetailsForInput');
			sinon.spy(ajwmd, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = ajwmd._getAddressDetailsForTransaction(tx, addresses);
			...
			ajwmd._getAddressDetailsForInput.callCount.should.equal(1);
			ajwmd._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var ajwmd = new AjwmService(baseConfig);
			sinon.spy(ajwmd, '_getConfirmationsDetail');
			ajwmd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			ajwmd.height = 300;
			var addresses = {};
			ajwmd._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			ajwmd._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				ajwmd.getDetailedTransaction.callCount.should.equal(1);
				ajwmd._getAddressDetailsForTransaction.callCount.should.equal(1);
				ajwmd._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			ajwmd._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				ajwmd.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from ajwmcore addresses', function() {
			var addresses = [
				ajwmcore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				ajwmcore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var ajwmd = new AjwmService(baseConfig);
			var strings = ajwmd._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var ajwmd = new AjwmService(baseConfig);
			(function() {
				ajwmd._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var ajwmd = new AjwmService(baseConfig);
			sinon.spy(ajwmd, '_normalizeAddressArg');
			ajwmd.maxAddressesQuery = 100;
			ajwmd.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			sinon.spy(ajwmd, '_normalizeAddressArg');
			sinon.spy(ajwmd, '_getAddressStrings');
			ajwmd.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
				ajwmd._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(ajwmd, '_normalizeAddressArg');
			sinon.spy(ajwmd, '_getAddressStrings');
			ajwmd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			ajwmd.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
				ajwmd._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(ajwmd, '_normalizeAddressArg');
			sinon.spy(ajwmd, '_getAddressStrings');
			ajwmd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			ajwmd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			ajwmd.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
				ajwmd._getAddressStrings.callCount.should.equal(1);
				ajwmd.getAddressTxids.callCount.should.equal(1);
				ajwmd._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(ajwmd, '_normalizeAddressArg');
			sinon.spy(ajwmd, '_getAddressStrings');
			sinon.spy(ajwmd, 'getAddressTxids');
			ajwmd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			ajwmd.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
				ajwmd._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(ajwmd, '_normalizeAddressArg');
			sinon.spy(ajwmd, '_getAddressStrings');
			sinon.spy(ajwmd, '_paginateTxids');
			ajwmd._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(ajwmd, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			ajwmd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			ajwmd.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
				ajwmd._getAddressStrings.callCount.should.equal(1);
				ajwmd.getAddressTxids.callCount.should.equal(1);
				ajwmd._getAddressDetailedTransaction.callCount.should.equal(2);
				ajwmd._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			ajwmd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			ajwmd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			ajwmd.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			ajwmd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			ajwmd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			ajwmd.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			ajwmd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			ajwmd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			ajwmd.getAddressSummary(address, options, function(err, summary) {
				ajwmd._paginateTxids.callCount.should.equal(1);
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
				ajwmd.getAddressTxids.callCount.should.equal(1);
				ajwmd.getAddressBalance.callCount.should.equal(1);
				ajwmd._paginateTxids.args[0][1].should.equal(0);
				ajwmd._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			ajwmd.getAddressSummary(address, options, function(err, summary) {
				...
				ajwmd.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					ajwmd._paginateTxids.callCount.should.equal(0);
					ajwmd._normalizeAddressArg.callCount.should.equal(2);
					ajwmd.getAddressTxids.callCount.should.equal(1);
					ajwmd.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			ajwmd.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				ajwmd._paginateTxids.callCount.should.equal(1);
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
				ajwmd.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			ajwmd._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		var blockhex = '';
		it('will give rpc error from client getblockhash', function(done) {
			...
			ajwmd.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(ajwmd, '_tryAllClients');
			ajwmd.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			ajwmd.getRawBlock(blockhash, function(err) {
				should.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			ajwmd.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			ajwmd.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			ajwmd.getBlock(0, function(err) {
				ajwmd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as ajwmcore object from height', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			sinon.spy(ajwmd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			ajwmd.getBlock(0, function(err, block) {
				should.not.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(ajwmcore.Block);
				done();
			});
		});
		it('will getblock as ajwmcore object', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			sinon.spy(ajwmd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			ajwmd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			ajwmd.getBlock(hash, function(err, block) {
				should.not.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			ajwmd.getBlock(0, function(err, block) {
				...
				ajwmd.getBlock(0, function(err, block) {
					should.not.exist(err);
					ajwmd._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			ajwmd.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var ajwmd = new AjwmService(baseConfig);
			sinon.spy(ajwmd, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			ajwmd.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			ajwmd.getBlockHeader(10, function(err) {
				ajwmd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			ajwmd.getBlockHeader(blockhash, function(err) {
				ajwmd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			ajwmd.getBlockHeader(0, function(err) {
				ajwmd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			ajwmd.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				ajwmd._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			ajwmd.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				ajwmd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			ajwmd._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				ajwmd._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			ajwmd._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			ajwmd._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			sinon.spy(ajwmd, '_maybeGetBlockHash');
			ajwmd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			ajwmd.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				ajwmd._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			ajwmd.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				ajwmd.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					ajwmd._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			ajwmd.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = ajwmcore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			ajwmd.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			ajwmd.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			ajwmd.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = ajwmcore.Transaction();
			(function() {
				ajwmd.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getRawTransaction('txid', function(err) {
				should.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			ajwmd.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getTransaction('txid', function(err) {
				should.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			ajwmd.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				ajwmd._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(ajwmcore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			ajwmd.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				ajwmd._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			ajwmd.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			ajwmd.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			ajwmd.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.node.getNetworkName = sinon.stub().returns('testnet');
			...
			ajwmd.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			ajwmd.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			ajwmd.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				ajwmd.getSubsidy.callCount.should.equal(1);
				ajwmd.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			ajwmd.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.spawn = {};
			ajwmd.spawn.process = new EventEmitter();
			ajwmd.spawn.process.kill = sinon.stub();
			ajwmd.stop(done);
			ajwmd.spawn.process.kill.callCount.should.equal(1);
			ajwmd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			ajwmd.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			ajwmd.spawn.process.kill.callCount.should.equal(1);
			ajwmd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			ajwmd.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var ajwmd = new AjwmService(baseConfig);
			ajwmd.shutdownTimeout = 300;
			ajwmd.spawn = {};
			ajwmd.spawn.process = new EventEmitter();
			ajwmd.spawn.process.kill = sinon.stub();
			ajwmd.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var ajwmd = new AjwmService(baseConfig);
			sinon.spy(ajwmd, '_normalizeAddressArg');
			sinon.spy(ajwmd, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			ajwmd.nodes.push({
			...
			ajwmd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				ajwmd._getBalanceFromMempool.callCount.should.equal(1);
				ajwmd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			ajwmd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			ajwmd.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			ajwmd.getJsonBlock(0, function(err) {
				should.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			ajwmd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			ajwmd.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(2);
				ajwmd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			ajwmd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
				ajwmd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var ajwmd = new AjwmService(baseConfig);
			var emitter = new EventEmitter();
			ajwmd.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(ajwmd.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var ajwmd = new AjwmService(baseConfig);
			var emitter = new EventEmitter();
			ajwmd.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ajwmd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			ajwmd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			ajwmd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ajwmd.unsubscribeBalance(emitter1);
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ajwmd.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			ajwmd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var ajwmd = new AjwmService(baseConfig);
			...
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ajwmd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ajwmd.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var ajwmd = new AjwmService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ajwmd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(ajwmd, 'unsubscribeBalanceAll');
			ajwmd.unsubscribeBalance(emitter1);
			ajwmd.unsubscribeBalanceAll.callCount.should.equal(1);
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ajwmd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ajwmd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			ajwmd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			ajwmd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			ajwmd.unsubscribeBalanceAll(emitter1);
			ajwmd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ajwmd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			ajwmd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(ajwmd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var ajwmd = new AjwmService(baseConfig);
			sinon.spy(ajwmd, '_notifyBalanceSubscriber');
			ajwmd._getAddressesFromTransaction = sinon.stub().returns(addresses);
			ajwmd.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				ajwmd._notifyBalanceSubscriber.callCount.should.equal(1);
				ajwmd._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			ajwmd._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				ajwmd.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			ajwmd._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			ajwmd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
		...
		it('will give account info', function(done) {
			var accountInfo = {
				...
				"storage": {
					...
					"03fac7d0ed7c64751ajwm5d6cc9bf86a98b2109c28bb87d95632c271bc24d05ba": {
					...
					"79defe99e7df4f6fbd8f65cd320f5c35dbbe574701c957ca05266895c64b10ed": {
						"59a0de65a1280c37ed0f0bd1467785774c27e0c93d057d2035ad954ajwma919b0": "7159635532473166346b57475a58676b573268364555705850354b5134786656"
					},
					...
					"8568295bd78d84b5440d8871431707ab29ce98253271648b8ajwm1c1f7b198a66": {
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getDgpInfo(function(err, result) {
				should.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getMiningInfo(function(err, result) {
				should.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			ajwmd.getStakingInfo(function(err, result) {
				should.exist(err);
				ajwmd._tryAllClients.callCount.should.equal(1);
