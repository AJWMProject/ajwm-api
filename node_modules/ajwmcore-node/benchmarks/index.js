...
var ajwmd = require('../').services.Bitcoin({
  node: {
    ...
    function(next) {
      ...
      function bitcoinGetTransactionNative(deffered) {
        ...
        var hash = fixtureData.txHashes[c];
          ajwmd.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      suite.add('ajwmd getblock (native)', bitcoindGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('ajwmd getblock (json rpc)', bitcoindGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('ajwmd gettransaction (native)', bitcoinGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('ajwmd gettransaction (json rpc)', bitcoinGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
